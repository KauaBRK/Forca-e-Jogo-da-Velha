...
int ticTacToeCheckWins(char ticTacToe[3][3], char winner) {
  /*FUNCAO QUE CHECA SE HOUVE GANHADOR*/
...
int ticTacToeSwitchCase(char ticTacToe[3][3] /*Jogo da velha*/ , int xOption /*Vê se X ja jogou*/ ) {
  ...
  fflush(stdin);
  switch (lineChoose) {
    /*.ESTE PARAMETRO ATRIBUIRA O CARACTER X PARA A PRIMEIRA PESSOA QUE ESCOLHEU E
  ...
  case 1:
    if (ticTacToe[0][0] == '1' && ticTacToe[0][0] != '\n') {
      ...
      if (xOption == 1) {
        /*VEJO SE O JOGADOR ATUAL É X OU 0*/
      ...
      /* CASO O LOCAL JA ESTEJA OCUPADO EXIBE A MENSAGEM */
...
int main() {
  ...
  /*VENDEDOR.*/
  for (i = 1; i < 10; i++) {
    /*ZERANDO O STRUCT TODO PARA MANIPULAR SE O VETOR ESTA OCUPADO OU NAO*/
    players[i].winner0 = 0;
    players[i].winnerX = 0;
  }
  while (continue_ == 1 || continue_ == 2) {
    /*.ENQUANTO O JOGADOR QUER JOGAR O LOOP CONTINUA OU ATE O PAR FOR MENOR QUE 10.
    		//ESTA PRIMEIRA PARTE SERVE APENAS PARA EU ZERAR TODOS OS PARAMETROS ANTERIORES*/
    if (pair < 10) {
      /*VEJO SE JA CHEGOU NO NUMERO MAXIMO DO VETOR*/
      ...
      while (winner == 'n') {
        /*.ENQUANTO O VENCEDOR FOR N OU NONE O JOGO CONTINUA.*/

        printf("\n\n\tDupla %d\n", pair); /*MOSTRANDO A DUPLA ATUAL*/
        xOption = ticTacToeSwitchCase(ticTacToe, xOption);
        /*DECLARO A VARIAVEL XOPTION AO MESMO TEMPO QUE CHAMO O SWITCHCASE
      ...
      if (winner != 'n') {
        /*SE WINNER NAO FOR MAIS NONE O GANHADOR RECEBE WINNER*/
        ...
        if (continue_ == 2) {
          /*SE QUER TROCAR DE DUPLA ADICIONO UMA DUPLA NO VETOR*/
          pair += 1;
        }
        system("cls");
      } else {
      ...
      /*SE JA LOTOU DE PLAYERS ENCERRA*/
  ...
  printf("\n\tObrigado por jogar :)\n\n\n\t############\n\t###PLACAR###\n\t############\n\n");
  for (i = 1; i < 10; i++) {
    /*ESTE FOR VAI AO MESMO TEMPO IMPRIMIR RESULTADOS E VER SE O VETOR ESTA VAZIO*/
    if (players[i].counter == 0) {
      /*SE O VETOR COUNTER ESTIVER ZERADO SIGNIFICA QUE ELE ESTA VAZIO ENTAO PARA O FOR*/
      break;
    }
